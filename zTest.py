...
from MeasModels import RangeMeas
...
x0 = np.array([6750, 0, 0, 7])

nx = len(x0)

t = np.arange(0, calc_period(x0, mu), 1)  # sec
...
Ws = [[[np.nan, np.nan], [np.nan, np.nan]]]
epsilons = [[np.nan, np.nan]]
